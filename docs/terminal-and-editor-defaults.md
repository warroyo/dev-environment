...
## Git signing over a forwarded agent

On Linux, git signs through
[`git-ssh-sign`](../dotfiles/dot_local/bin/executable_git-ssh-sign), a wrapper
that runs `ssh-keygen` with `SSH_AUTH_SOCK` removed.

`ssh-keygen -Y sign` probes the agent *before* falling back to the on-disk key.
When a laptop lid closes, the forwarded agent socket is still held open by
sshd — so `connect()` succeeds but the reply never arrives, and `git commit`
hangs indefinitely with no error. That's a very live risk here: commits get made
from tmux on the server over an agent forwarded from a laptop that closes
constantly.

The fix is a per-machine, passphrase-less signing key (generated by
`server-bootstrap.sh`) plus the wrapper, so signing never depends on the agent.
Signing stays **off** until you register the key on GitHub:

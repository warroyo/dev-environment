...
Also part of the general layer — installed on all three machines (release
tarballs on the server, Homebrew on both Macs):

| Tool | What it gives you |
|---|---|
| [`kubectl`](https://kubernetes.io/docs/reference/kubectl/) | The cluster CLI |
| [`krew`](https://krew.sigs.k8s.io) | kubectl's plugin manager — `kubectl krew install <plugin>` |
| [`kubectx` / `kubens`](https://github.com/ahmetb/kubectx) | Switch cluster context and namespace without editing kubeconfig |

`50_kube.sh` aliases **`kubectl` → `k`** on every machine that has kubectl, and
gives the alias kubectl's own tab-completion (`compdef k=kubectl`), so
`k get po<TAB>` behaves identically to `kubectl get po<TAB>`.

Unlike `find`/`grep`, aliasing here is safe: `k` shadows no existing command,
so there's no incompatible-flag trap.

Two things worth knowing:

- **kubectl's completion is generated by the binary**, and generating it costs
  roughly 200ms of shell startup. `50_kube.sh` caches the output to
  `~/.zcompcache-kubectl` and regenerates only when the kubectl binary is newer
  than the cache — the same trick `50_completion.sh` uses for `compinit`.
...
default repos. Both downloads are sha256-verified against the checksums
published with the release, since these are raw binaries over plain HTTPS with
no package manager checking signatures — same reasoning as the `kubectl` block
above them. `provision/verify-server.sh` checks all of it, including that
`kubectl krew` actually resolves and that `k` is aliased in an interactive
shell.
